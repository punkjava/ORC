						...
						if($val['area']== $val2['area'])
						{
							if($val3['source'] == $val['area'])
							{
								...
								if($date3 <= $date1)
								{
										...
										echo "window.location.href = 'http://127.0.0.1:8800/orc/server/check2.php?uq='+ uq </script>";
								}
								else
								{
									$str = "Updation Failed..As Souce does not matched";
									echo "<script>window.location.href = 'http://127.0.0.1:8800/orc/server/check3.php?uq= ".$str."'</script>";
